...
<body>
        ...
        <table class="tablelist">
            ...
            <tbody>
                <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
                    <td>
                        <input name="check" type="checkbox" value="<?php echo ($vol["admin_id"]); ?>" />
                    </td>
                    <td><?php echo ($vol["admin_id"]); ?></td>
                    <td><?php echo ($vol["admin_user"]); ?></td>
                    <td><?php echo (date('Y-m-d H:i:s',$vol["admin_lasttime"])); ?></td>
                    <?php if($vol["admin_id"] == 1 ): ?><td>超级管理员</td>
                    <td>超管太牛</td>
                    <?php else: ?>
                    <td><?php echo ($vol["role_name"]); ?></td>
                    <td><a href="/index.php/Admin/Admin/setAuth/admin_id/<?php echo ($vol["admin_id"]); ?>" class="tablelink">修改</a> <a href="/index.php/Admin/Admin/del/admin_id/<?php echo ($vol["admin_id"]); ?>" class="tablelink" onclick="return del()"> 删除</a></td><?php endif; ?>
    ...
    <script type="text/javascript">
        ...
        $('#dels').click(function(){
            ...
            location.href = "/index.php/Admin/Admin/dels/id/"+ids;
        });
